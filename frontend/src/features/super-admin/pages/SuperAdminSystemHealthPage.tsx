...
export function SuperAdminSystemHealthPage() {
  ...
  if (error || !health) {
    return <ErrorState onRetry={() => void load()} title="Unable to load system health." />;
  }
  ...
  return (
    <>
      <PageHeader
        action={
          <button className="rounded-xl bg-[#111827] px-4 py-3 text-sm font-black text-white" onClick={() => void load()} type="button">
            Refresh
          </button>
        }
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard label="Database" value={health.databaseStatus} tone="success" />
      ...
      <div className="mt-6">
        <Panel title="Last Health Check">
          <div className="flex items-center justify-between rounded-xl border border-[#edf1f7] p-4">
            <p className="font-bold text-[#09172d]">{new Date(health.checkedAt).toLocaleString()}</p>
