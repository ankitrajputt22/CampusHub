...
export function SuperAdminCollegeDetailsPage() {
  ...
  async function setStatus(status: 'ACTIVE' | 'INACTIVE' | 'BLOCKED') {
    ...
    const confirmed = window.confirm(
      `Change ${details.college.name} to ${status}?`,
    );
    if (!confirmed) return;
  ...
  if (error || !details) {
    return (
      <ErrorState onRetry={() => void load()} title="Unable to load college." />
    );
  }
    ...
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        <StatCard
          ...
          value={details.college.verifiedStudentsCount}
        />
        <StatCard
          label="Active Listings"
          value={details.college.activeListingsCount}
        />
      ...
      <div className="mt-6 grid gap-5 xl:grid-cols-[360px_1fr]">
        <Panel title="College Summary">
          <div className="space-y-3 text-sm">
            ...
            <Detail
              label="Location"
              value={`${details.college.city}, ${details.college.state}, ${details.college.country}`}
            />
            <Detail
              label="Email Domain"
              value={details.college.emailDomain ?? 'Optional'}
            />
            <div>
              ...
              <div className="mt-1">
                <StatusPill value={details.college.status} />
              </div>
            </div>
          </div>
          <div className="mt-5 grid gap-2">
            <button
              className="h-10 rounded-xl bg-emerald-700 text-sm font-black text-white"
            ...
            <button
              className="h-10 rounded-xl border border-[#d7dfeb] text-sm font-black"
            ...
            <button
              className="h-10 rounded-xl border border-rose-200 bg-rose-50 text-sm font-black text-rose-700"
